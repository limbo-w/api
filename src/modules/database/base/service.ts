...
import { OrderType, QueryTrashMode, TreeChildrenResolve } from '../constants';
...
> {
    ...
    async paginate(
        ...
        // 排序
        if (!isNil(options.orderBy)) {
            qb.addOrderBy(
                `${this.repository.getQBName()}.${options.orderBy}`,
                options.orderByDirection ?? OrderType.ASC,
            );
        }
